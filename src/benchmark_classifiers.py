...
def get_models() -> dict:
    ...
    return {
        ...
        "LinearSVC": (
            LinearSVC(
                C=1.0, max_iter=5000, random_state=42, dual=True,
            ),
            False,
        ),
        "SGDClassifier": (
            SGDClassifier(
                loss="hinge", alpha=0.0001, max_iter=1000, random_state=42,
            ),
            True,
        ),
    }
...
def run_benchmark(data_dir: Path):
    ...
    if n_subjects == 0:
        print("Error: No usable recordings found. Ensure unicorn-data/ contains valid CSV files.")
        return
    ...
    print(f"Total pooled samples: {len(all_y)} ({np.sum(all_y==1)} engaged, {np.sum(all_y==0)} disengaged)")
    ...
    for model_name, (model_template, partial_fit) in models_dict.items():
        ...
        for subj_idx, (X, y) in enumerate(subject_data):
            ...
            skf_subj = StratifiedKFold(n_splits=n_folds_subj, shuffle=True, random_state=42)
            fold_accs = []
    ...
    # Print summary table sorted by pooled accuracy
    print("\n\n")
    print("=" * 130)
    ...
    sorted_results = sorted(results.items(), key=lambda x: x[1]["pooled_accuracy"], reverse=True)
    ...
    candidates = [
        (name, r) for name, r in sorted_results
        if r["pooled_accuracy"] >= 0.90 and r["single_latency_ms"] < 2.0
        and r["has_proba"]
    ]
    ...
    if candidates:
        ...
        print(f"  Pooled CV acc:  {best_r['pooled_accuracy']:.1%} +/- {best_r['pooled_std']:.1%}")
        print(f"  Within-subj:    {best_r['within_accuracy']:.1%} +/- {best_r['within_std']:.1%}")
        print(f"  Latency:        {best_r['single_latency_ms']:.3f} ms (single), {best_r['batch_latency_ms']:.3f} ms (batch)")
        ...
        if fast_models:
            ...
            print(f"  Pooled CV:      {best_fast[1]['pooled_accuracy']:.1%} +/- {best_fast[1]['pooled_std']:.1%}")
            print(f"  Within-subj:    {best_fast[1]['within_accuracy']:.1%} +/- {best_fast[1]['within_std']:.1%}")
        ...
        print(f"a 20+ classifier weighted ensemble. Realtime features (169-dim absolute")
        print(f"power, no baseline) are inherently less discriminative.")
        print(f"\nLATENCY FINDINGS (valid regardless of accuracy):")
        print(f"  - sklearn RF/ExtraTrees: ~13ms (tree traversal dominates)")
        print(f"  - LightGBM:              ~0.25ms (50-400x faster than sklearn RF)")
        print(f"  - XGBoost:               ~0.18ms (similar to LightGBM)")
        print(f"  - Linear models (LDA/LR/SVM/SGD): ~0.02ms (fastest)")
        print(f"  - MLP (64-128 units):    ~0.02-0.03ms")
        print(f"\nFor the real-time path, replacing sklearn RF-200 with LightGBM-50")
        print(f"would reduce inference from 13.4ms to 0.25ms (54x speedup) with")
        print(f"comparable accuracy on realtime features.")
